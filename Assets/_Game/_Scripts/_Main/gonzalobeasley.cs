...
public class gonzalobeasley : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (Transform sylviarich in transform)
            albertamoyer.Add(sylviarich.gameObject);
...
private void jenniedonahue(int num)
    ...
    private void chandralayton()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (loadPixels != null)
            {
                Texture2D tex = new Texture2D(jamesrhodes, katelynhorton, TextureFormat.RGBA32, false);
                tex.filterMode = FilterMode.Point;
                tex.wrapMode = TextureWrapMode.Clamp;
                tex.LoadRawTextureData(loadPixels);
                tex.Apply(false);

                transform.GetChild(i).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, jamesrhodes, katelynhorton), Vector2.zero, 100);
            }
        }
    }
    ...
    public void freidalanders(int index)
    {
        ...
        {
            alexdrake.yvetteangel = index;
        }
        else
        {
            alexdrake.yvetteangel = -1;
        }

        alexdrake.deniceweiss = bertiedelarosa + index.ToString();
